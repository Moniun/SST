...
def parse_args():
    ...
    # 混合精度和内存优化（仅通过TrainingArguments控制，不手动干预）
    ...
    parser.add_argument("--eval_steps", type=int, default=1000, help="每多少步进行一次评估")
    ...
    parser.add_argument("--metric_for_best_model", type=str, default="meteor", help="用于选择最佳模型的指标")
...
class CustomTrainer(Trainer):
    def __init__(self, *args, cmd_args=None, **kwargs):
        ...
        # 初始化评估所需资源（只加载一次）
        self.tokenizer = AutoTokenizer.from_pretrained(cmd_args.model_name_or_path)
        self.sbert_model = sbert_model  # 预加载Sentence-BERT模型
    
    def compute_metrics(self, eval_pred):
        """计算评估指标（弱化token级指标，强化语义级指标）"""
        predictions, labels = eval_pred
        predictions = torch.argmax(torch.tensor(predictions), dim=-1)
        mask = labels != -100
        
        # 只保留F1分数作为唯一的token级指标
        ...
        # 重点增强METEOR和Sentence-BERT相似度指标计算
        meteor_scores = []
        sbert_similarities = []
        ...
        for pred, label, msk in zip(predictions, labels, mask):
            ...
            try:
                # 解码文本
                pred_text = self.tokenizer.decode(valid_pred_ids, skip_special_tokens=True).strip()
                label_text = self.tokenizer.decode(valid_label_ids, skip_special_tokens=True).strip()
                ...
                # 增强Sentence-BERT相似度计算
                if pred_text and label_text:
                    try:
                        # 使用batch编码提高效率，使用从命令行参数获取的batch_size
                        batch_size = self.cmd_args.per_device_eval_batch_size if hasattr(self.cmd_args, 'per_device_eval_batch_size') else 2
                        ...
                        similarity = calculate_cosine_similarity(embeddings[0], embeddings[1]).item()
        ...
        # 只返回必要的指标：一个token级(F1)和两个语义级(METEOR, S-BERT)
        return {
            "f1": f1_score,  # 唯一保留的token级指标
            "meteor": avg_meteor,  # 强化的语义级指标
            "sbert_similarity": avg_sbert_similarity  # 强化的语义级指标
        }
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        ...
        print(f"- F1分数 (Token级): {metrics.get('eval_f1', 0):.4f}")
        print(f"- METEOR分数 (语义级): {metrics.get('eval_meteor', 0):.4f}")
        print(f"- Sentence-BERT相似度 (语义级): {metrics.get('eval_sbert_similarity', 0):.4f}")
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        ...
        with torch.no_grad():
            # 使用框架默认的混合精度处理
            with autocast():
                # 创建模型输入的副本，避免修改原始inputs
                model_inputs = inputs.copy()
                
                # 如果ignore_keys不为None，过滤掉不需要的键
                if ignore_keys is not None:
                    model_inputs = {k: v for k, v in model_inputs.items() if k not in ignore_keys}
                
                # dialog_histories已在inputs中，无需额外处理
                outputs = model(**model_inputs)
        
        loss = outputs.loss
        logits = outputs.logits
        
        return (loss, None, None) if prediction_loss_only else (loss, logits, inputs.get("labels"))
        ...
        def __call__(self, features):
            ...
            for feature in features:
                filtered_feature = {
                    ...
                    'dialog_histories': feature.get('dialog_histories', [])  # 确保存在默认值
                }
